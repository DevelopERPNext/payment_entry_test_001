...
// -------- START  Stock and Invoice Details Fetcher for Sales Invoice Item ---------

//method: "payment_entry_test_001.payment_entry_test_001.payment_entry_py.",
...
frappe.ui.form.on('Sales Invoice Item', {
    item_code: function(frm, cdt, cdn) {
        ...
        if (row.item_code) {
            frappe.call({
                method: "payment_entry_test_001.payment_entry_test_001.payment_entry_py.get_item_stock_details",
                args: {
                    item_code: row.item_code
                },
                callback: function(r) {
                    if (r.message) {
                        frm.clear_table('item_stock_details_test_0004_aa');

                        r.message.stock_details.forEach((stock) => {
                            if (stock.actual_qty !== 0 || stock.reserved_qty !== 0) {
                                let child = frm.add_child('item_stock_details_test_0004_aa');
                                frappe.model.set_value(child.doctype, child.name, 'warehouse', stock.warehouse);
                                frappe.model.set_value(child.doctype, child.name, 'quantity_available', stock.actual_qty);
                                frappe.model.set_value(child.doctype, child.name, 'reserved_quantity', stock.reserved_qty);
                            }
                        });

                        frm.refresh_field('item_stock_details_test_0004_aa');
                    }
                }
            });

            frappe.call({
                method: "payment_entry_test_001.payment_entry_test_001.payment_entry_py.get_last_invoices",
                args: {
                    item_code: row.item_code
                },
                callback: function(r) {
                    if (r.message) {
                        frm.clear_table('item_last_invoices_test_0005_aa');

                        r.message.last_invoices.forEach((invoice) => {
                            let child = frm.add_child('item_last_invoices_test_0005_aa');
                            frappe.model.set_value(child.doctype, child.name, 'invoice_number', invoice.invoice_no);
                            frappe.model.set_value(child.doctype, child.name, 'invoice_date', invoice.posting_date);
                            frappe.model.set_value(child.doctype, child.name, 'last_sold_price', invoice.rate);
                        });

                        frm.refresh_field('item_last_invoices_test_0005_aa');
                    }
                }
            });
        }
...
// -------- END Stock and Invoice Details Fetcher for Sales Invoice Item ------------
...
// -------- START  Stock and Invoice Details Fetcher for Sales Order Item ---------
    ...
    item_code: function(frm, cdt, cdn) {
        ...
        if (row.item_code) {
            frappe.call({
                ...
                callback: function(r) {
                    if (r.message) {
                        frm.clear_table('item_stock_details_test_0004_aa_so');

                        r.message.stock_details.forEach((stock) => {
                            if (stock.actual_qty !== 0 || stock.reserved_qty !== 0) {
                                let child = frm.add_child('item_stock_details_test_0004_aa_so');
                                frappe.model.set_value(child.doctype, child.name, 'warehouse', stock.warehouse);
                                frappe.model.set_value(child.doctype, child.name, 'quantity_available', stock.actual_qty);
                                frappe.model.set_value(child.doctype, child.name, 'reserved_quantity', stock.reserved_qty);
                            }
                        ...
                        frm.refresh_field('item_stock_details_test_0004_aa_so');
                    }
                }
            });
            ...
            frappe.call({
                ...
                callback: function(r) {
                    if (r.message) {
                        frm.clear_table('item_last_invoices_test_0005_aa_so');

                        r.message.last_invoices.forEach((invoice) => {
                            let child = frm.add_child('item_last_invoices_test_0005_aa_so');
                            frappe.model.set_value(child.doctype, child.name, 'invoice_number', invoice.invoice_no);
                            frappe.model.set_value(child.doctype, child.name, 'invoice_date', invoice.posting_date);
                            frappe.model.set_value(child.doctype, child.name, 'last_sold_price', invoice.rate);
                        });

                        frm.refresh_field('item_last_invoices_test_0005_aa_so');
                    }
                }
            });
        }
...
// -------- END Stock and Invoice Details Fetcher for Sales Order Item ------------
